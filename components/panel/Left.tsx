    ...
    {state &&
      <LeftPortal>
        <motion.div initial={{marginRight: "100%"}} animate={panelState===true ? "openPanel" : "closePanel"} variants={variants} exit={exit} className="fixed inset-0 transform overflow-auto bg-transition z-90">
          ...
          <motion.div initial={{width:"320px",marginLeft:"-320px"}} animate={{width:"320px", marginLeft:0, transition:{duration:0.3}}} exit={{width:"320px",marginLeft:"-320px", transition:{duration:0.3}}} className="fixed h-screen z-100 bg-dark-600 shadow-md overflow-hidden overflow-y-auto text-white backdrop-blur bg-opacity-90">
            {children}
          </motion.div>
        </motion.div>
      </LeftPortal>  
