...
const Bottom = ({ state, close, children, title }) => {
  ...
  const variants = {
    ...
    closePanel: { 
      marginTop: "100vw",transition:{duration:0.5},
    }
  }
  const exit = {
    marginTop: "100vw",transition:{duration:0.5},
  ...
  return (
    ...
    {state &&
      <BottomPortal>
        <motion.div initial={{marginTop: "100vw",}} animate={panelState===true ? "openPanel" : "closePanel"} variants={variants} exit={exit} className="fixed inset-0 transform overflow-auto bg-dark-900 z-90">
