...
export default async (req, res) => {
  ...
  if (req.method === "POST")
  {
    ...
    try
    {
      ...
      if (existNickname) {
        return res.status(422).json({ message: 'User Nick Name already exists!', error: true });
      }
  ...
  {
    return res.status(405).json({message: "This request only supports POST requests", error:true})
  }
}