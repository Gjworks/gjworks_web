...
export async function GET(request: Request, segmentData: { params: Params }) {
  ...
  if (grantInfo.writeGrant && grantInfo.writeGrant.length > 0) {
    if(grantInfo.writeGrant.includes('member')) {
      if (!accessToken || accessToken === 'undefined') {
        response = {
          success: false,
    ...
    } else if(grantInfo.writeGrant.includes('admin')) {
      if (!userInfo?.isAdmin) {
        response = {
          success: false,
          errorCode: "INSUFFICIENT_PERMISSIONS",
    ...
    //비회원일 경우
    } else if(grantInfo.writeGrant.includes('guest')) {
    }else{
      
      if(userInfo && userInfo !== 'undefined') {
        const writeGrantIds = grantInfo.writeGrant.map((id) => parseInt(id, 10));
        const hasGrantPermission = userInfo.userGroups.some((group) => writeGrantIds.includes(group.groupId));
