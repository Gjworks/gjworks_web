  ...
  return (
    ...
    <div className="flex flex-wrap gap-8 max-w-screen-2xl mx-auto py-8 lg:py-20 px-3">
        ...
        <div className="flex-1 text-dark-100">
          <div className="text-primary-400 mb-10 text-base">Installation</div>
          <div className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white">Get started with Tailwind CSS</div>
          <div className="mt-4 text-lg text-dark-200">Tailwind CSS works by scanning all of your HTML files, <button onClick={() => setShowRight(!showRight)} className="text-primary-600 underline underline-offset-4 hover:text-primary-400">JavaScript components</button>, and any other templates for <code className="bg-primary-400 rounded-md p-1 text-sm text-primary-800">class</code> names, generating the corresponding styles and then writing them to a static <button onClick={() => setShowBottom(!showBottom)} >CSS file</button>.</div>
          <div className="py-5">
            <div className="rounded-lg bg-dark-500 bg-opacity-25 p-10"></div>
          </div>
        </div>
      </div>
      <Right state={showRight} close={closeRight} title="sdfsdfsdf">d1d</Right>
      <Bottom state={showBottom} close={closeBottom} title="234234234">d2d</Bottom>
      </PageWrap>
    </>
  )
}
